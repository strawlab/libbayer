...
/// For u8 source data.
pub fn read_exact_u8(r: &mut Read, buf: &mut [u8]) -> BayerResult<()> {
    r.read_exact(buf)?;
    Ok(())
}
...
pub fn read_exact_u16be(r: &mut Read, buf: &mut [u16]) -> BayerResult<()> {
    for i in 0..buf.len() {
...
pub fn read_exact_u16le(r: &mut Read, buf: &mut [u16]) -> BayerResult<()> {
    for i in 0..buf.len() {
